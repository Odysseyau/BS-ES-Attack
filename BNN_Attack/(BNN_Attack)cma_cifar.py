...
def box_cai1(img,k):
    ...
    #     cv2.rectangle(img_kuang, (x, y), (x + w, y + h), (0, 255, 0), 2)
    if k % 100 == 0 :
        fig1 = plt.figure(1)
        plt.subplot(1,3,1)
...
def universal_perturbation2(model,theta_train, p_train, n, dataset, max_iter_df=1000):
    ...
    # Shuffle the dataset
    # np.random.shuffle(dataset)#不打乱数据，进行维度的比较
    data = CIFAR()
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    device = torch.device("cpu")
    # state_dict = torch.load("./mobilenetv2-c5e733a8.pth")
    ...
    for k in range(0, 1000):  # num_images
        ...
        # try:
        #     box_a = box_cai1((cur_img).reshape(32,32,3), k)
        #     box_b = box_cai2((cur_img).reshape(32,32,3), k)
        #     pix_a = box_a[2]*box_a[3]
        #     pix_b = box_b[2]*box_b[3]
        #     if pix_a > pix_b and pix_a!=32*32:
        #         box = box_a
        #     elif pix_b > pix_a and pix_b!=32*32:
        #         box = box_b
        #     else:
        #         box =(5,5,18,18)
        #     # if box_a[2]*box_a[3]<650:
        #     #     box_a=(5,5,20,20)
        # except Exception as e:
        #     box=(0,0,32,32)
        # print(box)

        #cur_img = np.transpose(cur_img, (0, 3, 1, 2))

        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        device = torch.device("cpu")

        # cur_img = Variable(torch.from_numpy(cur_img).to(device).float())
        # model = reactnet(num_classes=CLASSES)
        # model = nn.DataParallel(model).cuda()


        # print(" ", cur_img.shape)   #(1,32,32,3)
        # img = (img - mean) / std
        ...
        # print(label,result)
        # num, pert_v, result, label, pert_image, n = cifar_choise_cma.nattack(box, model,  cur_img, max_iter=1000)
        # label, result, pert_image,pert_v =  cifar_choise_cma.deepfool(cur_img, model, num_classes=10, overshoot=0.02, max_iter=50)
        # num =0
        ...
        #     box = (0, 0, 32, 32)
        #     n = box[2] * box[3] * 3
        #     theta, iter, dorta, p, pert_v, result, pert_image, num, label, n = cifar_choise_cma.smr1(box, model,
        #                                                                                              [theta], dorta, p,
        #                                                                                              n, cur_img,
        #                                                                                              max_iter=max_iter_df)
        #     num+=num_qian
        ...
        # print("--------第%d个图像----------" % k)
        # print("原始类别为：%d,经过%d代的迭代后，label变为：%d，\n二范数结果为：%6f，无穷范数结果为：%6f，共查询%d次。" % (label,
        #                                                                           iter, result,
        #                                                                           torch.norm(pert_v),
        #                                                                           torch.norm(pert_v, float('inf')),
        #                                                                           num))
        # # if k % 100 == 0:
        #     # print("--------第%d个图像----------" % k)
        #     # print("原始类别为：%d,经过%d代的迭代后，label变为：%d，\n二范数结果为：%6f，无穷范数结果为：%6f，共查询%d次。" % (label,
        ...
        #     fig2 = plt.figure(2)
        #     plt.subplot(1, 3, 1)
        #     plt.imshow((cur_img).reshape(model.image_size, model.image_size,model.num_channels))
        #     plt.title('%s'%classes[label])
        #     plt.subplot(1, 3, 2)
        #     plt.imshow(pert_image.reshape(model.image_size, model.image_size,model.num_channels))
        #     plt.title('%s'%classes[result])
        #     plt.subplot(1, 3, 3)
        ...
        # plt.imshow(man_show(pert_image.transpose(1, 2, 0)).reshape(32, 32, 3))  #
        plt.xticks([])  # 去掉横坐标值
        plt.yticks([])  # 去掉纵坐标值
...
if __name__ == "__main__":
    ...
    p=torch.zeros(n)
        ...
        #model_vd = VdCma(Fun())
    # theta, dorta, p, curimg, pert_image, results, num_query,l2_val,linf_val,succ_num,labels, ns = universal_perturbation2(model,theta, p, n, X)
    #
    # theta, iter, dorta, p, pert_v, result, pert_image, num = resnet_mr1.r1_binary(model, box, [theta], dorta, p, n,
    #                                                                               img, label_ini, max_iter=max_iter_df)
    ...
    pd.DataFrame(result_sum).to_csv('result/nattack.csv')
